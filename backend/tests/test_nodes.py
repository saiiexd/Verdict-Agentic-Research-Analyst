...
def test_research_node_ready():
    state = get_base_state()
    state["financial_data"] = {"mock": "data"}
    state["news"] = [{"title": "news1"}]
...
def test_refiner_node(mock_get_agent):
    ...
    state = get_base_state()
    state["report"] = "mock"
    state["critic_report"] = "mock_critic"
